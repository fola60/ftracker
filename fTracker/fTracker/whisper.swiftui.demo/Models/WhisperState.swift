...
class WhisperState: NSObject, ObservableObject, AVAudioRecorderDelegate {
    ...
    override init() {
        super.init()
        let model: Model = Model(name: "tiny.en", info: "(F16, 75 MiB)", url: "https://huggingface.co/ggerganov/whisper.cpp/resolve/main/ggml-tiny.en.bin", filename: "tiny.en.bin")
        loadModel(path: model.fileURL)
    }
...
//        messageLog += await WhisperContext.benchGgmlMulMat(nThreads: nThreads) + "\n"
    ...
    func transcribeSample() async -> Bool{
        if let sampleUrl {
